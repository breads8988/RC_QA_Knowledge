...
# Acceptance Criteria — Login (`LOGIN`)

> **Source note:** `RC-4` is the parent **Epic** (no detail of its own). Login requirements come from its child tickets: **RC-6** (login page), **RC-66** (login + forgot-password error handling), and **RC-26** (login entry points on the landing/home screen). UI grounded in `01_Features/login/screens/login.png`. Home-page, registration (RC-8) and tour-guide behaviours are owned by other features — only login-relevant entry points are covered here.
...
- **Coverage rule:** every `Critical` / `High` AC and every business rule has ≥1 Linked TC (filled above). Register: `01_Features/login/login_tc.md`.
