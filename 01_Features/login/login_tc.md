...
# Test Case Register — Login (`LOGIN`)

> Traced to AC spec `01_Features/login/login_ac.md`. Every TC names the `AC-LOGIN-NN` / `BR-LOGIN-NN` it verifies. TC priority = the verifying AC's criticality. New TCs: `Coverage: 🔵 Pending`, `Status: ⬜ Not Run` — detailed steps are written by the Cucumber automation layer.
...
| TC ID        | Test Scenario                | AC                          | Jira                                                   | Priority  | Coverage   | Cucumber Tag     | Preconditions                          | Test Data           | High-level Steps                                              | Expected Result                                                | Status    | Note                       |
| ------------ | ---------------------------- | --------------------------- | ------------------------------------------------------ | --------- | ---------- | ---------------- | -------------------------------------- | ------------------- | ------------------------------------------------------------ | -------------------------------------------------------------- | --------- | -------------------------- |
| TC-LOGIN-010 | Password masked by default, then toggle visibility | AC-LOGIN-08<br>BR-LOGIN-03 | [RC-6](https://motionscloud.atlassian.net/browse/RC-6) | 🟡 Medium | 🔵 Pending | `@TC-LOGIN-010` | On Login page | password: any       | 1. Type a password<br>2. Tap the reveal (eye) control<br>3. Tap it again           | The password is masked by default; after the first tap it becomes visible; after the second tap it is masked again             | ⬜ Not Run |                            |
